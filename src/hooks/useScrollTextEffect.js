...
const useScrollTextEffect = (
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (0 <= scrollProgress && scrollProgress <= 0.2) {
        targetElement.classList.remove("text-circle");
        targetElement.innerHTML = "<p></p>";
      }
      if (0.2 <= scrollProgress && scrollProgress <= 0.6)
      ...
      if (scrollProgress === 1) targetElement.innerHTML = `<p>${lastText}</p>`;
